...
```bash
go get github.com/rapidclock/web-octopus@v1.4.1
```
...
- **Publish workflow** (`.github/workflows/publish.yml`) runs only when a GitHub **Release** is published and triggers indexing on both the Go proxy and pkg.go.dev so new versions are discoverable quickly.
...
- Current release in this repository: **v1.4.1**.
